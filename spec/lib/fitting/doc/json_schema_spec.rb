...
describe Fitting::Doc::JsonSchema do
  ...
  context 'bug 4' do
    let(:fixture) { YAML.load(File.read('spec/fixtures/bugs/bug4/.fitting.debug.yml')) }
    ...
    it 'fixed' do
      ...
      res_debug = Fitting::Debug.report(0, json_schema, json_schema.logs)
      File.open('spec/fixtures/bugs/bug4/.fitting.test.yml', 'w') { |file| file.write(YAML.dump(res_debug)) }
      expect(res_debug['jsons'].size).to eq(fixture["jsons"].size)
      expect(res_debug['jsons']).to eq(fixture["jsons"])
      expect(res_debug['valid_jsons'].size).to eq(fixture["valid_jsons"].size)
      expect(res_debug['valid_jsons']).to eq(fixture["valid_jsons"])
  ...
  context 'bug 5' do
    let(:fixture) { YAML.load(File.read('spec/fixtures/bugs/bug5/.fitting.debug.yml')) }
    ...
    it 'fixed' do
      ...
      res_debug = Fitting::Debug.report(0, json_schema, json_schema.logs)
      File.open('spec/fixtures/bugs/bug5/.fitting.test.yml', 'w') { |file| file.write(YAML.dump(res_debug)) }
      expect(res_debug['jsons'].size).to eq(fixture["jsons"].size)
      expect(res_debug['jsons']).to eq(fixture["jsons"])
      expect(res_debug['valid_jsons'].size).to eq(fixture["valid_jsons"].size)
      expect(res_debug['valid_jsons']).to eq(fixture["valid_jsons"])
